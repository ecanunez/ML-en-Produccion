...
ANNEX_MD = DOCS_DIR / "annex_worldcup_validation.md"
...
OUTPUT_PDF = (
    OUTPUT_DIR
    / "Anexo A - Validacion externa Mundial 2026.pdf"
)

TEMP_MD = DOCS_DIR / "_annex_worldcup_build.md"
TEMP_HTML = DOCS_DIR / "_annex_worldcup_build.html"
...
def build_html():
    command = [
        ...
        "--toc",
        "--metadata",
        "title=Anexo A - Validación externa",
        "--metadata",
        "subtitle=Modelo campeón sobre la Copa Mundial de la FIFA 2026",
        "--metadata",
...
def main():
    if not ANNEX_MD.exists():
        raise FileNotFoundError(f"No se encontró el anexo: {ANNEX_MD}")
